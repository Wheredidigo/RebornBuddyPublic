...
namespace Loader
{
    public class EatMeWpfLoader : BotPlugin
    {
        #region Meta Data

        private const string PluginClass = "EatMeWpf.EatMe";
        private static readonly string PluginAssembly = Path.Combine(Environment.CurrentDirectory, @"Plugins\EatMeWpf\EatMeWpf.dll");
        ...
        private static void LoadPlugin()
        {
            lock (ObjLock)
            {
                ...
                InjectedOnInitialize.Invoke(Plugin, null);
                if (PluginManager.GetEnabledPlugins().Any(n => n == "Eat Me Wpf"))
                {
                    InjectedOnEnabled.Invoke(Plugin, null);
                }
            }
